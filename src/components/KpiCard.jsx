...
export default function KpiCard({ label, value, change, prefix = '', suffix = '', icon, delay = 0, decimals = 0 }) {
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl border p-5 transition-all duration-300 sage-border-glow"
      style={{
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-[13px] font-medium" style={{ color: '#7A9B88' }}>{label}</span>
        {icon && <span className="text-lg">{icon}</span>}
      </div>
      <div className="text-[28px] font-bold tracking-tight tabular-nums leading-tight" style={{ color: '#2A3B32' }}>
